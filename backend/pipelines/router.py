...
async def _next_copy_scenario_name(owner_id: str, client_name: str) -> str:
    existing_names = {
        p.scenario_name
        for p in await list_owner_plans(owner_id)
...
async def _ensure_distinct_scenario_name(
...
async def list_plans(owner_id: str = "anonymous") -> list[PlanSummary]:
    ...
    plans = await list_owner_plans(owner_id)
...
async def delete_plan(plan_id: str, req: DeletePlanRequest) -> dict[str, bool]:
    ...
    if plan is None or plan.owner_id != req.owner_id:
        raise HTTPException(status_code=404, detail="Plan not found")
    deleted = await delete_plan_store(plan_id)
    if not deleted:
        raise HTTPException(status_code=404, detail="Plan not found")
    return {"deleted": True}
...
async def update_scenario_name(plan_id: str, req: UpdateScenarioNameRequest) -> PlanSummary:
    ...
    existing_names = {
        p.scenario_name
        for p in await list_owner_plans(req.owner_id)
...
@router.patch("/plans/{plan_id}/fields")
async def update_plan_fields(plan_id: str, req: UpdateFieldsRequest) -> dict[str, Any]:
    """Update individual plan fields via patch operations."""
    plan = await get_plan(plan_id)
    if plan is None or plan.owner_id != req.owner_id:
        raise HTTPException(status_code=404, detail="Plan not found")

    patches = [
        PatchOp(op="set", path=u.path, value=u.value, confidence=1.0)
        for u in req.updates
    ]
    updated, result = apply_patches(plan, patches)
    if result.rejected:
        rejected_paths = [path for path, _ in result.rejected]
        raise HTTPException(
            status_code=400,
            detail=f"Invalid fields: {', '.join(rejected_paths)}",
        )

    updated.updated_at = datetime.now(timezone.utc)
    await store_plan(updated)
    payload = updated.model_dump(mode="json")
    payload["display_name"] = _plan_display_name(updated)
    payload["client_name"] = _plan_client_name(updated)
    return payload
...
async def copy_plan(plan_id: str, req: CopyPlanRequest) -> PlanSummary:
    ...
    scenario_name = (
        ...
        else await _next_copy_scenario_name(req.owner_id, client_name)
    )
    scenario_name = await _ensure_distinct_scenario_name(
